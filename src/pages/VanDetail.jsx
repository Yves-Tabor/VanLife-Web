...
function VanDetail() {
    ...
    return (
        <>
            <div className={`${theme === 'light' ? 'bg-[#FEF6EA]' : 'bg-black'} w-full flex flex-col items-center min-h-screen`}>
                <div className={`container mx-auto px-4 py-8 w-[85%] flex flex-col items-center justify-center`}>
                    <Link to={`../Vans?${search}`} className={`w-full mb-6 hover:text-gray-800 flex items-start`}>
                        <span className="mr-2">«</span><span className={`hover:underline ${theme === 'light' ? 'text-black' : 'text-white'}`}> Back to {type.charAt(0).toUpperCase() + type.slice(1)} vans</span>
                    </Link>
                    ...
                    <div className="bg-white rounded-lg p-6 shadow-md w-full md:w-[60%] lg:w-[50%]">
                        ...
                        <div className="pt-4">
                            ...
                            <p className={`text-gray-600 leading-relaxed text-black`}>{van.description}</p>
                        </div>
